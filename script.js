document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    if (openTerminalBtn && terminalModal) {
        ...
        terminalInput.addEventListener('keydown', async (e) => {
            if (e.key === 'Enter') {
                ...
                const originalCommand = terminalInput.value; // Keep case for display
                
                // Add command to history/output
                addToTerminal(`user@portfolio:~$ ${originalCommand}`);
                terminalInput.value = '';
    ...
    function addToTerminal(text, isHtml = false) {
  ...
  <span class="text-success">about</span>    - Who am I?
  <span class="text-success">skills</span>   - My technical skills
  <span class="text-success">projects</span> - Fetch my GitHub repos
  <span class="text-success">contact</span>  - How to reach me
  <span class="text-success">clear</span>    - Clear terminal
  <span class="text-success">exit</span>     - Close terminal`, true);
                break;
            case 'about':
                addToTerminal(`I am Tarun Patil, a Java Full-Stack Developer passionate about building scalable applications. I love solving complex problems and learning new technologies.`);
                break;
            case 'skills':
                addToTerminal(`Frontend: HTML, CSS, JS, React, Next.js
Backend: Java, Spring Boot, Node.js
Database: MySQL, MongoDB
DevOps: Git, Docker, AWS`);
                break;
            case 'projects':
                addToTerminal('Fetching repositories from GitHub...', true);
                try {
                    // Using a placeholder username for demo purposes. 
                    // In a real scenario, this would be 'tarunpatil' or similar.
                    // Since I don't know the exact username, I'll use a popular one or just simulate it if fetch fails.
                    const username = 'tarunpatil'; // Replace with actual username
                    const response = await fetch(`https://api.github.com/users/${username}/repos?sort=updated&per_page=5`);
                    
                    if (response.ok) {
                        const repos = await response.json();
                        if (repos.length === 0) {
                            addToTerminal('No public repositories found.');
                        } else {
                            repos.forEach(repo => {
                                addToTerminal(`<a href="${repo.html_url}" target="_blank" class="text-info">${repo.name}</a> - ${repo.description || 'No description'}`, true);
                            });
                        }
                    } else {
                        // Fallback simulation if user doesn't exist or API limit reached
                        addToTerminal('Error fetching from GitHub API. Showing local featured projects:', true);
                        addToTerminal('- <span class="text-info">Verbavision</span>: AI Visual & Sound Generator', true);
                        addToTerminal('- <span class="text-info">Portfolio-v2</span>: This interactive website', true);
                    }
                } catch (error) {
                    addToTerminal('Network error. Please check your connection.');
                }
                break;
            case 'contact':
                addToTerminal(`Email: tarun@example.com
LinkedIn: linkedin.com/in/tarunpatil
GitHub: github.com/tarunpatil`);
                break;
            case 'clear':
            ...
            case '':
                break;
            default:
                addToTerminal(`Command not found: ${cmd}. Type 'help' for available commands.`);
        }
    }
